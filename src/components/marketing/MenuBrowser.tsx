...
export function MenuBrowser({ categories, items }: Props) {
  ...
  return (
    <div dir={rtl ? 'rtl' : 'ltr'} lang={locale}>
      {/* The heading lives inside the locale-aware component so the whole page flips,
          not just the item list. */}
      <div className="py-12 sm:py-16">
        <p className="eyebrow">{rtl ? 'القائمة' : 'The Menu'}</p>
        <h1 className="display mt-4 text-[clamp(2.75rem,8vw,5.5rem)]">
          {rtl ? 'كل ما نقدمه' : 'Everything we serve'}
        </h1>
        <p className="mt-5 max-w-lg leading-relaxed text-[var(--muted)]">
          {rtl
            ? `${available} صنفاً في ${categories.length} أقسام. الأسعار بالريال القطري.`
            : `${available} items across ${categories.length} sections. Prices in Qatari riyal.`}
        </p>
      </div>

      {/* sticky rail: language toggle + horizontally scrollable categories */}
      <div className="sticky top-[57px] z-40 -mx-5 border-b border-[var(--line)] bg-[var(--bg)]/92 px-5 backdrop-blur-md sm:-mx-8 sm:px-8">
        <div className="mx-auto flex max-w-6xl items-center gap-4 py-3">
          <div className="rail flex flex-1 gap-1 overflow-x-auto">
            {categories.map((c) => {
          ...
          <button
            ...
            className="shrink-0 rounded-full border border-ink/20 px-3.5 py-2 text-[0.72rem] tracking-wide transition-colors hover:border-brass hover:text-brass"
                ...
                return (
                  <li key={item.id} className="flex gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {item.is_signature && (
                        <span className="eyebrow mt-2 inline-block text-brass">Signature</span>
                      )}
                    </div>
                  </li>
                );
